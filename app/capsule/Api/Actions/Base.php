...
abstract class Base extends Controller {
   ...
   protected function respondToArray($document){
...
//处理后台置顶的作品排序
protected function respondWithDocumentTop($document, $topdata,$workIds,$page,$statusCode = 200, $headers = [])
    {
       
       // pr($topdata);
       //先做数据的过滤  再添加置顶的数据

       $newarray = $document->toArray();
/*
       if(!empty($workIds)){
          //做数据过滤  过滤掉不在workIds里面的数据  也就是当前选中话题的作品数据
        foreach ($array['data'] as $key => $value) {
             if(!in_array($value['id'], $workIds)){
     ...
     $newarray['data'] = array_values($array['data']);
*/  
     if($page==1){
       $toparray = $topdata->toArray();
      

       foreach ($toparray as $key => $value) {
           array_unshift($newarray['data'],$toparray[$key]);
       }
        
        }
       //pr($array);
       return $this->respondWithArray($newarray, $statusCode, $headers);
