...
class Huanxin extends Base {
	...
	public function run()
	{
		$users=$this->user->where("uuid","=",Null)->where("mobile","!=","")->select("id","mobile")->get();
		
		//return $users[0];
		/******************/
		foreach($users as $user){
			//return $user;exit;
			$res=$this->createUser($user->id,md5($user->id.$user->mobile));
			DB::update("UPDATE users SET uuid = '".$res["entities"][0]["uuid"]."' WHERE id = ".$user->id);
		}
		return "end huanxin";
		/******************/		
	}
