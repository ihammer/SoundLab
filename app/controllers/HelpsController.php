<?php
class HelpsController extends FormController{
...
//		DB::table('users')->where('id',"=", $_POST["id"])->update(array('recommend' => $_POST["recommend"],'recommendtime'=>date('Y-m-d H:i:s',time())));
	 ...
	 public function upsave()
   {
	...
	$data=array(
		"status"=>$_POST["status"],
		"callinfo"=>$_POST["callinfo"],
		'updated_at'=> date('Y-m-d H:i:s'),
	);
	
   	DB::table('helps')->where('id', $_POST["id"])->update($data);
    return Redirect::to(action($this->controller . '@index'));
   }
	 ...
	 public function edit($id)
     {
          ...
          return View::make('form.helpsedit', compact('model','ca'));
     }
