...
$works=json_decode(file_get_contents("http://123.57.1.143/api/work/".$_GET["id"]."/show"),1);
...
$comment=json_decode(file_get_contents("http://123.57.1.143/api/comment/".$_GET["id"]."/show"),1);
$tmpfile=json_decode(file_get_contents("http://123.57.1.143/api/work/".$_GET["id"]."/tmpfile"),1);
...
<html>
<head>
<title><?php echo $works["data"]["title"];?></title>
<meta charset="utf-8" >
...
<body>
<div style="display:none;"><img width="100%" height="100%" id="bgImg" src="<?php echo $works["data"]["cover"];?>" /></div>
...
<audio src="<?php echo $tmpfile["tmpfile"];?>" preload="true" controls="true" id="video1" hidden>    
</audio>
<div id="main_b">
...
<div style="display:none;cursor:pointer;width:80%;position:fixed;bottom:10px;left:10%" id="download"><img width="100%" src="/test/helen/7.png"></div>
...
$(function(){
	...
	function tangmu(text,j){
		...
		mathHeight = Math.round(Math.random()*$('#danmu').innerWidth()*0.60)+"px";
